...
Ahora crea un archivo `src/Main.elm` en tu editor, y copia el código del [ejemplo de los botones](https://elm-lang.org/examples/buttons) dentro de él.
...
Esto inicializa un servidor en [`http://localhost:8000`](http://localhost:8000). Desde ahí puedes navegar a cualquier archivo Elm que tenga `main` para ejecutarlo. Corre `elm reactor`, abre el link de localhost, y busca tu archivo `src/Main.elm` en tu navegador.
...
Este comando produce los mismos mensajes que has visto en el editor online y con `elm reactor`. Hemos puesto años de trabajo en ellos, pero por favor [reporta cualquier mensaje confuso o poco útil](https://github.com/elm/error-message-catalog/issues). Estoy seguro de que aún podemos mejorarlos.
